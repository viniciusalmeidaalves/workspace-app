...
def get_ipmet_weather(city_id='224'):
    ...
    current_app.logger.info("Buscando previsão do tempo (simulada) - Pode estar em cache após a primeira vez.")
    try:
        today = datetime.now()
...
def get_g1_news():
    ...
    current_app.logger.info("Buscando notícias G1 (simuladas) - Pode estar em cache após a primeira vez.")
    try:
        news = [
...
def search():
    ...
    if query:
        ...
        useful_links = UsefulLink.query.filter(
            (UsefulLink.title.ilike(search_pattern)) | 
            (UsefulLink.category.ilike(search_pattern))
        ).order_by(UsefulLink.title).all()
...
def intranet():
    ...
    # NOVO: Busca todos os eventos do banco de dados
    events = Event.query.all()
    # Converte os eventos para o formato que o FullCalendar entende (JSON)
    # Importante: fullcalendar exige que as datas sejam ISO 8601 (ex: "2023-10-27T10:00:00")
    events_for_calendar = [event.to_fullcalendar_format() for event in events]
    # Usaremos json.dumps para passar a string JSON para o template
    ...
    return render_template('intranet.html', 
                           ...
                           now=now,
                           banners=banners,
                           events_json=events_json) # NOVO: Passa os eventos formatados para o template
